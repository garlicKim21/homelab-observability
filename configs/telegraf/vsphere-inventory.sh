...
if [[ -n "$vm_paths" ]]; then
  vm_total=$(echo "$vm_paths" | wc -l | tr -d ' ')

  # govc vm.info -json accepts multiple paths
  vm_info=$(govc vm.info -json $vm_paths 2>/dev/null || echo '{}')

  # Count power states using jq
  powered_on=$(echo "$vm_info" | jq '[.virtualMachines[] | select(.runtime.powerState == "poweredOn")] | length' 2>/dev/null || echo 0)
  powered_off=$(echo "$vm_info" | jq '[.virtualMachines[] | select(.runtime.powerState == "poweredOff")] | length' 2>/dev/null || echo 0)
